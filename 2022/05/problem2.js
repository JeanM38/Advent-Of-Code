...
for (let i = 0; i < lines.length; ++i) {
    ...
    if (/^[a-z]+$/.test(lines[i][0]) && lines[i] !== '') {
        ...
        /* Get stack items to remove */
        let itemsToRemove = [...stacks[guideline.from]].splice(0, guideline.amount);

        /* Add theme to target */
        stacks[guideline.to] = [...itemsToRemove, ...stacks[guideline.to]];

        /* Remove them from previous stack */
        stacks[guideline.from].splice(0, guideline.amount);
    }
}
